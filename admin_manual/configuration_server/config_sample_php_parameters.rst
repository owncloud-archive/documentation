...
ownCloud supports loading configuration parameters from multiple files. You can 
add arbitrary files ending with ``.config.php`` in the ``config/`` directory, 
for example you could place your email server configuration in 
``email.config.php``. This allows you to easily create and manage custom 
configurations, or to divide a large complex configuration file into a set of 
smaller files. These custom files are not overwritten by ownCloud, and the 
values in these files take precedence over ``config.php``.

.. The following section is auto-generated from 
.. https://github.com/owncloud/core/blob/stable8/config/config.sample.php
