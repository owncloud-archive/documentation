...
because it does not store locks, and data can disappear from the cache at any 
time
...
Additional Configuration for Background Job Cron and occ Command Line Tool
--------------------------------------------------------------------------

.. For 8.1 only

If you have enabled the Background Job Cron or want to use the occ command line 
tool, an additional PHP configuration option is needed for ownCloud 8.1 when using APC 
or APCu:

    apc.enable_cli = 1

This can be either configured in your main ``php.ini``, or within the file ``apc.ini`` or 
``apcu.ini`` shipped by your distribution. The location of this file depends on your 
used distribution. On Debian/Ubuntu those files are located in:
...
It is important that you configure this for the PHP cli package and not for the 
Webserver specific configuration of PHP.
