...
Two further examples are manually expire passwords and configuring the number of days that have to pass since the last change before the password expires.
These ensure compromised passwords can be immediately changed and that users change passwords on a semi-regular basis, making them harder to be cracked.
