...
Linux distribution packages (from our `Open Build Service`_) have been divided into 
multiple packages for ownCloud 8.2: ``owncloud-deps`` and ``owncloud-files``. 
...
* `<https://download.owncloud.org/download/repositories/8,2/owncloud/>`_
