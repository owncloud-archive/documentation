...
OC Package
~~~~~~~~~~
The three OC_* classes are part of the API for applications. The OC_Files class can be used for retrieving the actual content of a file or directory (by creating a ZIP file for example). The OC_FileSystem class is deprecated and simply forwarding the method calls to OC\\Files\\FileSystem. Finnaly the OC_FileSystemView class is an extension of the View class and again doesn't add any extra functionality.
...
In the Storage interface are a few methods declared for retrieving objects from the Cache package. The Cache class is used for the Metadata cache for the filesystem. The cache needs to be retrieved by using the getFileInfo method from the FileSystem class.
...
.. figure:: ../../images/sequenceFileSystem.jpg
    :width: 500px
    :align: center
